...
module.exports = async (Bot, member, interaction) => {
  if (member.level < 2) {
    try {
      await interaction.reply(
        'You need to be at least level 2 to use this command.'
      );
    } catch (err) {
      console.error(err);
    }
  ...
  try {
    if (interaction.member.id === recipient.id)
      return await interaction.reply(notices.invalidSelf);
    if (member.lastStar === today)
      return await interaction.reply(notices.invalidMax);
  } catch (err) {
    console.error(err);
  }

  let recipientData = await Bot.db
    .collection('members')
  ...
  let recipientUpdates = updateLevel(
    recipientData,
    expAddends.starred,
  ...
  const botEmbed = new MessageEmbed()
    ...
    .setDescription(
      `${recipient.displayName} got a star from ${interaction.member.displayName}!\n\nThey also got +100 EXP as a bonus. :sparkles:`
    )
    .setColor(YELLOW)
